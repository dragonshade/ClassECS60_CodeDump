...
class HeapObj{
    public:
    HeapObj(unsigned int i, unsigned int d){index=i; dist=d;}
    HeapObj(){ index=-1; dist=-1;}
    unsigned int index;
    unsigned int dist;
    bool operator<(HeapObj & rhs){ return dist<rhs.dist;}
};
...
public:
    ...
    unsigned int calCost(Vertex* pivot, Edge* pt);
    unsigned int calCostOrigin(Vertex* origin, Edge* pt);
    HeapObj FindMinUnknown(BinaryHeap<HeapObj> &heap);
