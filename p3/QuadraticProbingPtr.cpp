         ...
         */
        template <class HashedObj>
        void QuadraticPtrHashTable<HashedObj>::insert( HashedObj & x )
        {
                // Insert x as active
            const HashedObj* x_prime=new HashedObj(x);
                
            int currentPos = findPos( x );
            if( isActive( currentPos ) )
                return;
            array[ currentPos ] = x_prime;
...
/* 7*/      return currentPos;
         ...
         */
        template <class HashedObj>
        void QuadraticPtrHashTable<HashedObj>::remove(HashedObj & x )
        {
            int currentPos = findPos( x );
            if( isActive( currentPos ) )
         ...
         */
        template <class HashedObj>
        const HashedObj & QuadraticPtrHashTable<HashedObj>::find(HashedObj & x ) const
        {
            int currentPos = findPos( x );
            if(isActive( currentPos ))
            {
            HashedObj temp = *array[currentPos];
            return temp;
            }
