...
void Router::Dijkstra(Vertex* start, Vertex* goal){
     ...
     // BinaryHeap heap(nFlights);
      ...
      while(pt!=0)
      {
         ...
         if(newCost< vArray[ptIndex]->dist)
        {
          vArray[ptIndex]->dist=newCost;
          vArray[ptIndex]->pv = pt->edgeNum; //denote previous vertice using previous edge/flight num
          vArray[ptIndex]->previous=start->index; //set previous vertice
          //Dump the (vertice, dist) pairs into the heap;
    
          unsigned int temp = ((ptIndex << 16) | newCost);
          heap->insert(temp);
        }
       ...
       // |      |    index|       |     dist |  = minUnknown
       //  Byte 4   Byte 3   Byte 2   Byte 1
       //  index =  minUknown >> 16,    dist = (unsigned short)minUknown
  do{
       //FIND MINIMUM DISTANCE IN UNKNOWN VERTICES MARK AS KNOW
      unsigned int minUnknown=FindMinUnknown();
      unsigned int minIndex = (minUnknown >>16);
       vArray[minIndex]->known=true;
     
       
       //
       
       //clocktime at new pivot is equal to trip departuretime + the dist(duration of travel)
       vArray[minIndex]->clock=start->clock+vArray[minIndex]->dist;
       
       pt=vArray[minIndex]->first;
       while(pt!=0)
       {
            ptIndex=pt->index;
            if(!vArray[ptIndex]->known)  //do the following only to unknown vertex
           {
             
            newCost=calCost(vArray[minIndex],pt);
            //if this Total_cost is less than the cost update
            if(newCost< vArray[ptIndex]->dist)
            {
               vArray[ptIndex]->dist=newCost;
               vArray[ptIndex]->pv = pt->edgeNum; //denote previous vertice using previous edge/flight num
               vArray[ptIndex]->previous=minIndex; //Set previous vertice index
               unsigned int temp = ((ptIndex << 16) | newCost);
               heap->insert(temp);
            }
           }
...
////////////////////////////////////////////////////////////////
unsigned int Router::FindMinUnknown(){

      while(vArray[((heap->findMin()) >> 16)]->known)
